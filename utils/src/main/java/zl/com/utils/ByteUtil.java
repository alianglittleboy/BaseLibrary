...
public class ByteUtil {
    ...
    public static byte[] subBytes(byte[] src, int begin, int count) {
        if(null==src||src.length<begin+count){
            return null;
        }
...
//                | ((bb[index + 1] & 0xff) << 8) | ((bb[index + 0] & 0xff) << 0)));
     ...
     * @param index
     */
    public static byte[] putLong(long x, int index) {

        Log.d("MainActivity", "putLong: " + x);

        byte[] longByte = new byte[64];
        longByte[index + 7] = (byte) (x >> 56);
        longByte[index + 6] = (byte) (x >> 48);
        longByte[index + 5] = (byte) (x >> 40);
        longByte[index + 4] = (byte) (x >> 32);
        longByte[index + 3] = (byte) (x >> 24);
        longByte[index + 2] = (byte) (x >> 16);
        longByte[index + 1] = (byte) (x >> 8);
        longByte[index + 0] = (byte) (x >> 0);
        return longByte;
    }
